...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indexstr) + str.slice(indexstr + value.length);
}
...
function formatTime(minutes, seconds) {
  ...
  const padStmin = min.padStart(2, 0);
  const padStsec = sec.padStart(2, 0);
  return padStmin.concat(':', padStsec);
}
...
function isPalindrome(str) {
  ...
  const streverse = lowRegStr.split('').reverse().join('');
  return streverse === lowRegStr;
}
...
function reverseWords(str) {
  ...
  let reserverd = '';
  for (let i = 0; i < strsplit.length; i += 1) {
    reserverd += `${strsplit[i].split('').reverse().join('')} `;
  }
  return reserverd.trimEnd();
}
